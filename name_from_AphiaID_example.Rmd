...
for (i in 1:length(df$AphiaID)) {
  if (isTRUE(nchar(df$AphiaID[i])>0)==TRUE) {
...
```{r}
#check if names match match
##add column to hold check
df<-add_column(df,scientificName_AphiaIDName_Match="",.after="name_from_AphiaID")

##run check itself (tolower() is present to account for variations in capitalization)
for (i in 1:length(df$AphiaID)) {
  if (isTRUE(tolower(df$scientificName[i])==tolower(df$name_from_AphiaID[i]))==TRUE) {
